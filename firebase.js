...
async function startMultiplayer() {
  try {
    console.log(
      '📡 Multiplayer: Fetching Firebase modules via dynamic import...'
    );
    ...
    const packPos = (p, r, s, l, v) =>
      `${p.x.toFixed(2)},${p.y.toFixed(2)},${p.z.toFixed(2)},${r.x.toFixed(3)},${r.y.toFixed(3)},${r.z.toFixed(3)},${s.toFixed(2)},${l ? 1 : 0},${v === 'helicopter' ? 1 : 0}`;
    const unpackPos = (csv) => {
    ...
    function createOtherPlaneMesh(uid, forcedColor) {
      ...
      const color =
        forcedColor !== undefined
          ? forcedColor
          : ChillFlightLogic.getPlaneColor(uid);
      ...
      const hcp = new THREE.Mesh(new THREE.BoxGeometry(4, 3, 3), windowMat);
      hcp.position.set(0, 0.5, -3);
      helicopterModel.add(hcp);
      const tail = new THREE.Mesh(new THREE.BoxGeometry(1.5, 1.5, 10), mat);
      tail.position.set(0, 1, 8);
      helicopterModel.add(tail);
      const mainRotor = new THREE.Group();
      mainRotor.position.set(0, 3, 0);
      helicopterModel.add(mainRotor);
      const mb1 = new THREE.Mesh(new THREE.BoxGeometry(24, 0.2, 1.5), bladeMat);
      mainRotor.add(mb1);
      const mb2 = mb1.clone();
    ...
    onChildAdded(playersRef, (snapshot) => {
      ...
      if (data.lastSeen) {
        ...
        const now =
          Date.now() +
          (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0);
    ...
    console.warn(
      '📡 Multiplayer: Running in offline mode (Connection blocked or unavailable).'
    );
...
function initMultiplayerUI() {
  ...
  const handleNameChange = (e) => {
    ...
    if (e.target === nameInput && splashInput)
      splashInput.value = e.target.value;
...
if (ChillFlightLogic.ENABLE_MP) {
  ...
  const hideUI = () => {
    ...
    const splashNameContainer = document.getElementById(
